...
from helper.continuous_traces import get_channel_count, filtr
...
#	xpos: x position on the probe, calcualted from the template. requires an accurate site_positions. averages template from 100 spikes.
    ...
    units = {}
    for i in np.arange(1,np.shape(cluster_id)[0])[:5]:
        if cluster_id[i][0].split('\t')[1] != 'garabge' :#:or cluster_id[i][0].split('\t')[1] == 'unsorted' :#if it is a 'good' cluster by manual sort
            ...
            for channel in range(site_positions.shape[0])[:-10]:
...
def isiViolations(directory,time_limits=None,tr=.0015,tc=0):
    ...
    Nums = np.zeros(np.shape(cluster_IDs)[0])
...
def cluster_signalToNoise(directory,time_limits=None,filename='experiment1_100-0_0.dat',sigma=5.,number_to_average=250,plots=False):
    ...
    spike_times_path = os.path.join(directory,'spike_times.npy')
    params_path = os.path.join(directory,'params.py')
    raw_data_path = os.path.join(directory)
    ...
    if os.path.isfile(spike_clusters_path):
        ...
        spike_clusters = np.load(spike_templates_path)
    try:
        rawdata = np.memmap(os.path.join(raw_data_path,filename), dtype=np.int16, mode='r')
    except:
    ...
    params = read_kilosort_params(params_path)
    spike_times = np.load(spike_times_path) / float(params['sample_rate'])
    channels = get_channel_count(raw_data_path)
    ...
    for i,cluster_ID in enumerate(cluster_IDs):
        ...
        if all_spikes[-1] < time_limits[0] or all_spikes[0] > time_limits[1] or str(cluster_ID) not in data.keys():
            ...
            for channel in channels_with_template:
                ws = load_waveforms(os.path.join(raw_data_path,filename),channel,sub_times,channels=channels)

                mean = np.mean(ws,axis=0)       

                if time_limits[1]==1e7:
                    cunk = rawdata[int(650*float(params['sample_rate'].strip('.'))*channels):int(660*float(params['sample_rate'].strip('.'))*channels)][channel::channels] 
                else:
                    cunk = rawdata[int(time_limits[0]*float(params['sample_rate'].strip('.'))*channels):int(time_limits[1]*float(params['sample_rate'].strip('.'))*channels)][channel::channels] 
...
def masked_cluster_quality_sparse(spike_clusters,pc_features,pc_feature_ind,spike_times,spike_templates,time_limits=None,n_fet=3,fet_n_chans=9):
    ...
    for i,cluster_ID in enumerate(cluster_IDs):
        ...
        print ('\rcluster '+str(cluster_ID)+': # spikes:'+str(np.shape(fet_this_cluster)[0])+'  iso. distance:'+str(unit_quality[i])+'  contamination:'+str(contamination_rate[i])+'  '+str(i+1)+'/'+str(np.shape(unit_quality)[0]),)
    return cluster_IDs,unit_quality,contamination_rate,flda
...
def neuron_fig(clusterID,df,sortpath,filename='experiment1_102-0_0.dat',time_limits=None,timeplot_binsize=60.,neighbor_colors=["#67572e","#50a874","#ff4d4d"]):
    ...
    datapath=os.path.join(raw_data_path,filename)
    ...
    for ii,channel in enumerate(np.linspace(peak_y_channel-8,peak_y_channel+10,18)):
        if channel > 0:
            ws = load_waveforms(raw_data_path,channel,times,channels=channels)
            ws_bkd = load_waveforms(raw_data_path,channel,random_times,channels=channels)
            ...
            if ii > 3 and ii < 14:
                for nn,axis in enumerate(nearby_trio):
                        if np.shape(x_range)[0] == np.shape(np.mean(ws,axis=0)-yoff*(ii/2))[0]:
                            neighbor_ws = load_waveforms(raw_data_path,channel,np.random.choice(nearby_times[nn],100,replace=False),channels=channels)
