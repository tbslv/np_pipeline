...
#structures is a dictionary that defines the bounds of the structure e.g.:{'v1':(0,850), 'hpc':(850,2000)}
	...
	if 'processing' in nwb_data.keys():
		for probe in nwb_data['processing'].keys():
			if 'UnitTimes' in nwb_data['processing'][probe].keys():
				for i,u in enumerate(np.array(nwb_data['processing'][probe]['UnitTimes'].keys())):
					if u != 'unit_list':
						nwb_id.append(nwbid)
						...
						if 'ypos' in nwb_data['processing'][probe]['UnitTimes'][u].keys():
							ypos.append(np.array(nwb_data['processing'][probe]['UnitTimes'][u]['ypos']))
							has_ypos = True
						else:
							ypos.append(None)
							has_ypos = False				
						if 'depth' in nwb_data['processing'][probe]['UnitTimes'][u].keys():
							...
							if has_ypos:
								...
								depth.append(None)
						if 'xpos' in nwb_data['processing'][probe]['UnitTimes'][u].keys():
...
def classify_waveform_shape(df,plots=False,save_plots='no',basepath=''):
...
def plot_waveform_classification(durations, PTratio, repolarizationslope, df,save_plots, basepath=''):
