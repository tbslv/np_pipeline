...
def plot_PSTH_gaussian_cv(selection, times_cut_un,trials=None,samplingrate=30000,cv = None,start_rel=None,end_rel=None,stepsize=None,binsize=None):
	...
	times_cut = np.sort(times_cut_un)
	fit = calculate_bandwidth(times_cut,cv=cv) # start=start_rel,end=end_rel
	pdf = build_pdf(times_cut,start_rel,end_rel,samplingrate=30000,kernel = 'gaussian',bw = fit.best_estimator_.bandwidth,stepsize=stepsize)


	bins = np.arange(start_rel*samplingrate,end_rel*samplingrate,binsize*samplingrate/1000)
	x_grid = np.arange(start_rel*samplingrate,end_rel*samplingrate,stepsize*samplingrate/1000)
	response = response_detection_core(pdf,samplingrate=samplingrate)

	fig, ax1 = plt.subplots()
	ax1.plot(x_grid, pdf, linewidth=3, alpha=0.5, label='bw=%.1f // binsize=%.1f' % ((fit.best_estimator_.bandwidth/30),(bins[1]-bins[0])/30))
	ax2 = ax1.twinx()
...
def calculate_bandwidth(times_cut,samplingrate=30000,cv = 40):   
	bandwidths = np.array([50,100,150])*samplingrate/1000
	#bandwidths = np.arange(1,1500,5)*30

	grid = GridSearchCV(KernelDensity(kernel='gaussian'),{'bandwidth': bandwidths},cv=cv,n_jobs=10) # 20-fold cross-validation
